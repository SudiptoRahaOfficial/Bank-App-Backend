...
// middleware for authenticate system user
async function authenticateSystemUser(req, res, next) {
	// extracting authorization header from request
	const authorization = req.headers.authorization
	...
	try {
		...
		// returning failed response if not system user
		if (!user.systemUser) {
			return res.status(401).json({
				message: 'Unauthorized access, system permission required!',
				status: 'failed',
			})
		}

		// attaching authorized user's data to request
		req.user = { id: user._id }
